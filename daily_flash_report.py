...
if report_part == "Part 1 - Stock Markets Overview":
    ...
    general_index = ["^GSPC", "^IXIC", "^DJI", "^STOXX50E", "FTSEMIB.MI", "^GDAXI", "^N225",  "^NSEI", "^HSI", "SWDA.MI", "^FCHI", "^VIX"]
    ...
    index_data = yf.download(general_index, start=start_date, end=end_date, auto_adjust=False)
...
elif report_part =="Part 2 - Top and Worst Performers":    
    ...
    snp_prices = yf.download(tickers, in_start_date, in_end_date, auto_adjust=False)["Adj Close"]
    ...
    nadq_prices = yf.download(nasq_tickers, in_start_date, end_date, auto_adjust=False)["Adj Close"]
    ...
    stoxx50_prices = yf.download(stoxx50_tickers, in_start_date, end_date, auto_adjust=False)["Adj Close"]
...
elif report_part =="Part 3 - Interest Rates and Bond Yields":
    ...
    general_commodities_column_names = ["Gold", "Crude Oil", "Natural Gas", "Corn", "BTC EUR", "ETH EUR"]
    ...
    general_commodities_data_ret = general_commodities_data_ret[["GC=F", "CL=F", "TTF=F", "ZC=F", "BTC-EUR", "ETH-EUR"]]

    fig, ax = plt.subplots(figsize=(5, 2))  # Imposta la dimensione della figura
    ax.axis('tight')
    ax.axis('off')
    table = ax.table(cellText=general_commodities_data_ret.values, colLabels=general_commodities_column_names, rowLabels=general_commodities_data_ret.index, cellLoc='center', loc='center')
    table.scale(2.2, 1.8) 
    ...
    axs[0,0].set_title(f'USD / EUR:\n {round(currencies_data.iloc[len(currencies_data)-1:len(currencies_data),2], 4).tolist()}', fontsize=15)
    axs[0,1].plot(currencies_data.iloc[0:,1], color = "blue")
    axs[0,1].set_title(f'GBP / EUR:\n {round(currencies_data.iloc[len(currencies_data)-1:len(currencies_data),3], 4).tolist()}', fontsize=15)

    #second line
    axs[1,0].plot(currencies_data.iloc[0:,0], color = "blue")
    axs[1,0].set_title(f'CHF / EUR:\n{round(currencies_data.iloc[len(currencies_data)-1:len(currencies_data),1], 4).tolist()}', fontsize=15)
    axs[1,1].plot(currencies_data.iloc[0:,2], color = "blue")
    axs[1,1].set_title(f'EUR / JPY:\n {round(currencies_data.iloc[len(currencies_data)-1:len(currencies_data),0], 4).tolist()}', fontsize=15)
